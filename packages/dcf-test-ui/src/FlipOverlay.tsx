        ...
        {showResult ? (
          ...
          <>
            <div className="flip-sides">
              <button className={`flip-side ${side === 'H' ? 'active' : ''}`} onClick={() => setSide('H')} disabled={isFlipping}>HEADS</button>
              <button className={`flip-side ${side === 'T' ? 'active' : ''}`} onClick={() => setSide('T')} disabled={isFlipping}>TAILS</button>
            </div>

            <div className="flip-amount-section">
              <div className="flip-amount-display">{formatSol(amount, 3)} SOL</div>
              <div className="flip-quickpicks">
                {[0.001, 0.01, 0.1, 0.5, 1].map(v => (
                  <button key={v} className={`flip-qp ${Math.abs(v - amount) < 0.0001 ? 'active' : ''}`} onClick={() => setAmount(v)} disabled={isFlipping}>{v}</button>
                ))}
              </div>
            </div>

            {lastDryRun && (
              <div className="flip-cost">total: {formatSol(lastDryRun.total_cost, 3)} · win: {formatSol(lastDryRun.potential_win, 3)}</div>
            )}

            <button className="flip-btn primary" onClick={handleFlip} disabled={isFlipping}>
