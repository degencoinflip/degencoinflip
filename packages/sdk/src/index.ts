...
import { getBalance, getConnection, getWallet, getProvider, getProgram, loadKeypair, resetClients, setKeypair } from './anchor';
...
export { loadKeypair, setKeypair, getConnection, getWallet, getProvider, getProgram, resetClients } from './anchor';
...
export class DegenCoinFlip {
  ...
  constructor(opts: DegenCoinFlipOptions) {
    ...
    if (opts.keypair) {
      ...
      // Browser wallet adapter — for now, throw (Phase 3)
      throw new Error('Browser wallet adapter support coming in v0.2. Use keypair for now.');
    } else {
    ...
    // Initialize clients with the keypair
    resetClients();
  }
  ...
  async history(opts?: HistoryOptions): Promise<HistoryResult> {
    ...
    const token = await ensureAuth(this.keypair);
